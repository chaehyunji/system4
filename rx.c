...
void check_password() {
    ...
    if (strcmp(recv, correct_pw) == 0) {
        printf("[UNLOCKED] Access granted!\n");
        unlocked = 1;
        fail_count = 0;
    } else {
        fail_count++;
...
void sigio_handler(int signo) {
    ...
    if (!synced) {
        ...
        if (pattern == 0xAA) {
            ...
            bit_index = 0;
            memset(bits, 0, sizeof(bits));
            printf("\n[SYNC] Start pattern detected. Receiving password...\n");
        }
        return;
    }
...
int main() {
    ...
    if (ioctl(fd_clk, GPIO_IOCTL_ENABLE_IRQ, 0) < 0) {
        perror("ioctl ENABLE_IRQ");
        return 1;
    }

    printf("[RX] Waiting for password...\n");

    while (!unlocked) {
        time_t now = time(NULL);
        if (lock_until > 0 && now < lock_until) {
